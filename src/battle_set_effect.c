...
static void HandleSetEffectRemoveArgType(struct BattleCalcValues *cv, struct SetEffect *se)
{
    ...
    if (type == TYPE_ELECTRIC && gFieldTimers.terrain == B_TERRAIN_ELECTRIC)
        return;
    if (type == TYPE_FIRE && (GetWeather() & B_WEATHER_SUN)) // Suppressed Sun (Cloud Nine/Air Lock) doesn't protect the type
        return;
