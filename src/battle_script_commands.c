...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_BERRY_JUICE,     {   8,  10,  30,   _,   _,   _,   _,   _,   _,   _, } },
    ...
    { ITEM_ULTRA_BALL,      {   _,   1,   1,   1,   1,   4,   4,   5,   4,   5, } },
    ...
    { ITEM_PP_MAX,          {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    ...
    { ITEM_LUM_BERRY,       {   _,   _,   _,   _,   _,   _,   _,   1,   1,   1, } },
};
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, enum Ability abilityAtk, enum Ability abilityDef, enum HoldEffect holdEffectAtk)
{
    ...
    else
    {
        critChance  = (gBattleMons[battlerAtk].volatiles.focusEnergy != 0 ? 2 : 0)
                    ...
                    + ((abilityAtk == ABILITY_EXPERTISE && gMovesInfo[move].power <= 60) ? 2 : 0)
    ...
    if (critChance != CRITICAL_HIT_BLOCKED && (abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR || abilityDef == ABILITY_LEAF_GUARD))
    {
...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, enum Ability abilityAtk, enum Ability abilityDef, enum HoldEffect holdEffectAtk)
{
    ...
    if (gSideStatuses[battlerDef] & SIDE_STATUS_LUCKY_CHANT)
        critChance = CRITICAL_HIT_BLOCKED;
    else if (abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR || abilityDef == ABILITY_LEAF_GUARD)
    {
        if (recordAbility)
            RecordAbilityBattle(battlerDef, abilityDef);
        critChance = CRITICAL_HIT_BLOCKED;
