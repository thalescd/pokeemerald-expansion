...
AI_SINGLE_BATTLE_TEST("AI scores fixed damage moves correctly")
{
    ...
    PARAMETRIZE { move = MOVE_SONIC_BOOM, hp = 20; hitsToKO = 1; }
    PARAMETRIZE { move = MOVE_SONIC_BOOM, hp = 60; hitsToKO = 3; }
    PARAMETRIZE { move = MOVE_DRAGON_RAGE, hp = 40; hitsToKO = 1; }
    PARAMETRIZE { move = MOVE_DRAGON_RAGE, hp = 60; hitsToKO = 2; }

    GIVEN {
        AI_FLAGS(AI_FLAG_SMART_TRAINER); // Includes AI_FLAG_TRY_TO_2HKO, which scores OHKOs and 2HKOs
        ...
        TURN {
            ...
            if (hitsToKO == 1)
                SCORE_EQ_VAL(opponent, move, AI_SCORE_DEFAULT + BEST_DAMAGE_MOVE + FAST_KILL + BEST_EFFECT);
            else if (hitsToKO == 2)
                SCORE_EQ_VAL(opponent, move, AI_SCORE_DEFAULT + BEST_DAMAGE_MOVE + DECENT_EFFECT);
            else
